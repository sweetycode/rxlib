...
import { installScript } from "common-utils/utils/install";
...
export function MonacoEditor({
  ...
  const stateRef = useRef<{ value: string; language?: string }>({
    value: value,
    language: language,
  });
  const debouncedOnChange = useDebouncedCallback(() => {
    onChange &&
      editorRef.current &&
      onChange(
        (stateRef.current.value = editorRef.current.getModel().getValue())
      );
  }, 250)!;
  ...
  return (
    <div ref={containerRef} className={ccx("min-h-20 h-full", className)}>
...
export async function installMonacoEditor(): Promise<any> {
  return installScript(`${config.paths.vs}/loader.js`, async () => {
